...
<table>
    <tr>
        <td style="width: 50%;" valign="top">
            ...
            <span class="info">{{ trans('fi.invoice') }} #</span>{{ $invoice->number }}<br>
            <span class="info">{{ trans('fi.issued') }}</span> {{ $invoice->formatted_created_at }}<br>
            <br />
            <span class="info">Contact </span>{{ $invoice->custom->contact }}<br />
            <span class="info">Phone Number </span>{{ $invoice->custom->phone_number }}<br />
            <span class="info">Email </span>{{ $invoice->custom->email }}<br />
            <span class="info">Event </span>{{ $invoice->custom->event }}<br />
            <span class="info">Event Date </span>{{ $invoice->custom->event_date }} - {{ $invoice->custom->event_end_date }}<br />
            <span class="info">Venue </span>{{ $invoice->custom->venue }}<br />
			@if ($invoice->custom->event_notes) 
				<span class="info">Notes </span>{{ $invoice->custom->event_notes }}<br />
...
<!--             <span class="info">{{ mb_strtoupper(trans('fi.due_date')) }}</span> {{ $invoice->formatted_due_at }}<br><br> -->
            ...
            <span class="info">{{ trans('fi.bill_to') }}</span><br>{{ $invoice->client->name }}<br>
        ...
        <td style="width: 50%; text-align: right;" valign="top">
            ...
            @if ($invoice->user->email) Designer: {{ $invoice->user->name }}<br /><a href="mailto:{{ $invoice->user->email }}">{{ $invoice->user->email }}</a>@endif
        </td>
    </tr>
</table>
